...
#[tauri::command]
pub fn detect_workspaces(base_path: String) -> Result<Vec<WorkspaceInfo>, String> {
    ...
    for entry in entries {
        ...
        let git_branch = get_current_branch(&path);
...
pub fn get_workspace_info(path: String) -> Result<WorkspaceInfo, String> {
    ...
    let git_branch = get_current_branch(&ws_path);
    let last_modified = fs::metadata(&ws_path)
        .ok()
        .and_then(|m| m.modified().ok())
        .and_then(|t| {
            t.duration_since(std::time::SystemTime::UNIX_EPOCH)
...
fn get_current_branch(path: &PathBuf) -> Option<String> {
    Command::new("git")
